...
let
  options = rec {
    ...
    x86_64-windows = {
      ...
      sha256 = "136ggc8d7s09cb05lh9k41nzhachp15zfzl50fhp67j89wgvqz8f";
    };
  };
...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      if cfg.system == "x86-64-windows"
      then "https://galkowski.xyz/sbcl-${version}-${cfg.system}-binary.tar.bz2"
      else "mirror://sourceforge/project/sbcl/sbcl/${version}/sbcl-${version}-${cfg.system}-binary.tar.bz2";
    sha256 = cfg.sha256;
  };

  # dontUnpack = cfg.system == "x86-64-windows";

  nativeBuildInputs = lib.optionals (cfg.system != "x86-64-windows") [ makeWrapper ];
